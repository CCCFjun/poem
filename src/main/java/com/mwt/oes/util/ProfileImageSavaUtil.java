...
public class ProfileImageSavaUtil {
    ...
    public static String imageSave(MultipartFile newProfile, User user){
        // 根据Windows和Linux配置不同的头像保存路径
        String OSName = System.getProperty("os.name");
        String profilesPath = OSName.toLowerCase().startsWith("win") ? WINDOWS_PROFILES_PATH
                : LINUX_PROFILES_PATH;
        ...
        if (!newProfile.isEmpty()) {
            String profilePathAndNameDB = user.getUserImgSrc();
            // 默认以原来的头像名称为新头像的名称，这样可以直接替换掉文件夹中对应的旧头像
            String newImgSrc = profilePathAndNameDB;
            String newDistName = "";
            if(profilePathAndNameDB != null){
    ...
    public static String qiniuImageSave(MultipartFile file, User user) {
        String oldUserImgSrc = user.getUserImgSrc();
        if (oldUserImgSrc != null) {
            QiniuCloudUtil.deleteImage(oldUserImgSrc);
        }
        try {
            byte[] bytes = file.getBytes();
            String imageName = "profiles/" + UUID.randomUUID().toString();
